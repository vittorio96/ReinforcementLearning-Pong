...
class PongAgent(object):
    ...
    def __init__(self):
        ...
        self.experience_buffer_size = 2048
    ...
    def frame_preprocessing(self, frame_img):
        ...
        frame_img = frame_img[::2, ::2, 0]  # takes one element every four elements
    ...
    def train(self):
        ...
        frame_window = [initial_state, initial_state, initial_state]
        state = np.array(sum(frame_window[:self.frames_to_merge])).reshape(1, 80, 80)
        ...
        while episodes < self.max_episodes:
            ...
            frame_window.append(self.frame_preprocessing(observation))
            frame_window = frame_window[1:]
            next_state = np.array(sum(frame_window)).reshape(1, 80, 80)
